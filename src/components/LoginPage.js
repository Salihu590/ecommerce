...
const LoginPage = () => {
  ...
  const handleContinue = async (e) => {
    ...
    try {
      const response = await retryFetch(`http://localhost:5000/api/users/${email}`);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
      <div className="min-h-screen bg-blue-100 flex flex-col">
        <Header />
        <motion.div className="flex flex-col items-center w-full max-w-md mx-auto px-4 sm:px-6 py-12 text-center" variants={sectionVariants}>
          <img src="/images/logo.png" alt="EasyEats Logo" className="h-12 md:h-16 object-contain mb-8" />
          <h2 className="text-3xl sm:text-4xl font-bold flex items-center justify-center gap-2 mb-4">
            <span className="text-white bg-black px-3 py-1 rounded">Welcome</span>
            <span className="text-green-300"><Typing text=" to EasyEats" /></span>
          </h2>
          <p className="text-gray-600 text-base sm:text-lg mb-8">
            Type your e-mail or phone number to log in or create an EasyEats account.
          </p>
          ...
          <div className="w-full flex flex-col gap-4">
            ...
            >
              <img src="/images/facebook-logo.png" alt="Facebook Logo" className="h-5 w-5 object-contain" />
              Login with Facebook
            </button>
            <button
            ...
            >
              <img src="/images/google-logo.png" alt="Google Logo" className="h-5 w-5 object-contain" />
              Login with Google
            </button>
          </div>
        </motion.div>
